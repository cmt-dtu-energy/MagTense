function [vornb,vorvx,Aaug,baug] = polybnd_voronoi_OffSet(pos,bnd_pnts,d)
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
...
[Abnd,bbnd] = vert2lcon(bnd_pnts,1e-10*max(bnd_pnts(:))); 
...
%            '' ; 
%         end
        [A{i}(k,:),b{i}(k,:)] = pbisec_offset(pos(i,:), pos(vornb{i}(j),:),d);
        nn{i}(k,1) = vornb{i}(j) ;
    end
end
