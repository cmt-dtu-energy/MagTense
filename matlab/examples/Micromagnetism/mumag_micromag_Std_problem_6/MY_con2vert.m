...
if flg ==1
    V = [];
%     disp('Unable to locate a point within the interior of a feasible region.')
else
b = b - A*c;
...
[k,v1] = convhulln(D);
if v2 > v1
%    disp('Non-bounding constraints detected. (Consider box constraints on variables.)')
end
nr = unique(k(:));
...
V=V(I,:);
% if any(abs(V) >1 )
%    disp('POINTS OUTSIDE THE BOX!!') 
% end
end
% line(V(:,1),V(:,2),V(:,3),'marker','.','linestyle','none','markersize',16,'color','r') ;
return
