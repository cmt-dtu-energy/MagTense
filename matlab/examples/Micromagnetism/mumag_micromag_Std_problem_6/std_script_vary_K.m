...
title_str=[solver, ', ', num2str(tsteps),' steps'];
rng(9)
%% Physical Parameters
...
if contains(settings,'k')% soft anisotropy in left phase
    for i=1:nGrains/2
    %problem.u_ea(mesh_cart.iIn{i},:)=problem.u_ea(mesh_cart.iIn{i},:)*K0_soft/K0;
...
if useFinDif
    fnameSave=[fnameSave,'FinDifAexch'];
    solver=[solver,'FinDifAexch'];
end
    fnameSave=[fnameSave,'FinDifAexch_1DTest'];
    solver=[solver,'FinDifAexch_1DTest'];
save([fnameSave,'Solution.mat'],'solution')
%% Hysteresis Loop
...
title(title_str)
save(['std_script_vary_K\',fnameSave,'_',num2str(tsteps),'_tsteps.mat'],'solution','H','M') ;
savefig(['std_script_vary_K\',solver,'_',num2str(tsteps),'_',settings,'_no_precond'])
...
problem.SimulationName = [fnameSave,'_',num2str(tsteps),'_tsteps'];

oldPath = cd('../../../../../MagTense/matlab/micromagnetism_matlab_only_implementation');

problem.DemagTensorFileName= ...
    ['../../../MagTense_private/matlab/examples/Micromagnetism/std_script_vary_K/',...
    problem.DirectoryFilename,problem.SimulationName,'_demag.mat'];
...
if ~exist(problem.DemagTensorFileName,'file')
    ...
    InteractionMatrices.A2=sparse(D2A);
