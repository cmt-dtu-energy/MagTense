...
% indexItContains = cell(Nel*3,1);
indexItContainsRasmus = zeros(Nel*3*2,2);
...
for idim = 1:3
    ...
    % indexItContainsTemp = cell(Nel,1);
    for kb=1:Nel
        ...
        indxAB = Aindex(AcontainsB);
        indxBA = Aindex(BcontainsA);

        firstindx = [indxAB repmat(Bindex(kb),1,length(indxBA))];
        secondindx = [repmat(Bindex(kb),1,length(indxAB)) indxBA];

        % indexItContainsTemp{kb} = [firstindx',secondindx'];

        for i = 1:length(firstindx)
...
indexItContainsTrue = indexItContainsTrue(1:(k-1),:);
% indexItContainsTrue = cell2mat(indexItContains);
ItContains = logical(sparse(indexItContainsTrue(:,1),indexItContainsTrue(:,2),1,K,K)) ;
...
if 0 % Old Way
    ...
    thisBoolean(ItContains) =  not(ItsContained(ItContains)) ;
end
...
indexTheTs = cell(Nel,1);
indexTheDs = cell(Nel,1);
...
indexTheTsTrue = cell2mat(indexTheTs);
indexTheDsTrue = cell2mat(indexTheDs);

%These matrices could be logical
TheTs = sparse(indexTheTsTrue(:,1),indexTheTsTrue(:,2),1,K,Nel) ;
TheDs = sparse(indexTheDsTrue(:,1),indexTheDsTrue(:,2),1,K,Nel) ;
