function mesh = refineRectGrid_v6(X,Y,Z,x,y,z,NNs,L,VoronoiStruct,ndim)
%
...
%     textprogressbar = @(s) disp(s) ;
    ...
    for i=1:length( pos(:,1) ) % cycles over the cells of the coarse grid
        ...
        if isfield(VoronoiStruct,'ExtBorder')
        cell_tree = refineCell2( cell_tree, pts_central, ndim, criteria , VoronoiStruct.vorvx, VoronoiStruct.ExtBorder); 
        else
...
function [ref_tree] = refineCell2( ref_tree, pts_central, ndim, criteria , vorvx, ExtBorder)
                ...
                if hasGrandChildren
                    ...
                    for k=1:length(chrd)
                        if (strcmp(criteria.name,'voronoi_dist') || strcmp(criteria.name,'both'))
                            ...
                            NN = VoronoiDistanceRefinement(pp,pts_central,ndim,chrd,NN,k,vorvx,ExtBorder) ;    
                            else
...
function [cell_out] = getRefinedTree( pos, dims, ndim, L )
    ...
    for i=2:L
        ...
        for j=1:cell_out{i-1}.nCh   % loop over nodes on the same level of the tree
            ...
            if ndim==1
                [pos_tmp, dims_tmp] = getRefined1DRectangle( cell_out{i-1}.pos(j,:), cell_out{i-1}.dims(j,:) );
            elseif ndim==2
...
function [pos_out,dims_out] = getRefined1DRectangle( pos, dims)
    pos_out = zeros(2,3);
    dims_out = zeros(2,3);
    %%left rectangle
    pos_out(1,1) = pos(1) - dims(1)/4;
...
function NN = VoronoiDistanceRefinement(pp,pts_central,ndim,chrd,NN,k,vorvx,ExtBorder)
...
% pts_central : positions of the Voronoi generators
    ...
    if 0
        ...
        if isfield(vorvx,'data')
            ...
            if exist('ExtBorder')
            [~,NN(k)] = getTheDomains(pp(k,:),vorvx,ExtBorder) ;
            else
