...
class EvaluationPoints:
    ...
    def __init__(
        ...
        if line:
            _coords = np.vstack(
                (
                    np.mgrid[c[0] - r : c[0] + r : l_n[0], 0:0:1j, 0:0:1j],
...
class HalbachCylinder:
    ...
    Args:
        ...
        length: Height of each cylindrical tile.
    ...
    def __init__(
        ...
        r_o: float = 0.2,
        mag_res: int = 1,
        M_rem: np.ndarray | None = None,
        easy_axes: np.ndarray | None = None,
    ) -> None:
        ...
        tiles = magstatics.Tiles(
            ...
            mu_r_oa=1.17,
            dev_center=[
                (r_o - r_bore - eps) / (n_axial * mag_res),
                2 * np.pi / (n_segs * mag_res) - eps * np.pi,
                length - eps,
            ],
            M_rem=M_rem,
            easy_axis=easy_axes,
        ...
        for z in range(n_layers):
            z0 = -n_layers / 2 * length + z * length + length / 2

            for seg in range(n_segs):
                if easy_axes is None:
                    theta_seg = (
                        (2 * np.pi / n_segs) * seg + np.pi / n_segs + eps * np.pi
                    )
                    azimuth_opt = 2 * np.arctan2(np.sin(theta_seg), np.cos(theta_seg))

                for res_seg in range(mag_res):
                    theta0 = (
                        (2 * np.pi / (n_segs * mag_res)) * (seg * mag_res + res_seg)
                        + np.pi / (n_segs * mag_res)
                        + eps * np.pi
                    )
                    ...
                    for axial in range(n_axial):
                        for res_ax in range(mag_res):
                            idx = (
                                z * n_segs * mag_res * n_axial * mag_res
                                + seg * mag_res * mag_res * n_axial
                                + res_seg * mag_res * n_axial
                                + axial * mag_res
                                + res_ax
                            )

                            r0 = ((r_o - r_bore) / (n_axial * mag_res)) * (
                                axial * mag_res + res_ax + 0.5
                            ) + r_bore
                            tiles.center_pos = ([r0, theta0, z0], idx)
                            if M_rem is None:
                                tiles.M_rem = (B_rem / (4 * np.pi * 1e-7), idx)
                            if easy_axes is None:
                                tiles.set_easy_axis([np.pi / 2, azimuth_opt], idx)
    ...
    @property
    def remanence(self) -> np.ndarray:
        return self.tiles.M_rem[: self.n_hard_tiles]

    @property
    def easy_axes(self) -> np.ndarray:
        return self.tiles.u_ea[: self.n_hard_tiles]

    @property
    def pts_shim(self) -> EvaluationPoints | None:
        return self._pts_shim

    def perturb_config(self, B_rem, azimuth=None, polar=None) -> None:
        """
        Pertubation of the optimal configuration for the finite,
        ...
        self.reset()
        if azimuth is None:
            azimuth = self.l_azimuth_opt
        if polar is None:
            polar = np.ones(self.n_hard_tiles) * np.pi / 2
        mag_angles = np.concatenate(
            (np.expand_dims(polar, axis=1), np.expand_dims(azimuth, axis=1)), axis=1
        )
    ...
    def add_shim_magnets(
        self, n_layers, n_segs=None, shim_grid=None, mu_r=None, r_i=0.01
    ) -> None:
        """
        ...
        if shim_grid is None:
            ...
            self.shim_segs = list(shim_dict.keys())[
                list(shim_dict.values()).index(shim_grid)
            ]
        ...
        for z in range(n_layers):
            ...
            for grid_x in range(self.shim_grid[0]):
                for grid_y in range(self.shim_grid[1]):
                    off = [
                        (grid_x + 0.5) * size[0] - self.r_bore,
                        (grid_y + 0.5) * size[1] - self.r_bore,
                        z0,
                    ]
                    ...
                    if (
                        dist_c
                        < (self.r_bore - np.sqrt(size[0] ** 2 + size[1] ** 2)) ** 2
                        ...
                        self.tiles.size = ([size[0] / 2, size[1] / 2, length / 2], idx)
        ...
        self._pts_shim = EvaluationPoints(
            [pts_shim.shape[0], 1, 1], self.r_bore, coords=pts_shim
        )

    def set_shim_matrix(self, shim_mat) -> None:
        assert len(shim_mat) == self.n_shim
        self.reset()

        for i in range(self.n_shim):
            if shim_mat[i] == 1:
                self.tiles.incl_it = (1, self.n_hard_tiles + i)

    def set_one_shim_magnet(self, idx) -> None:
        self.reset(shim_magnets=False)
        self.tiles.incl_it = (1, self.n_hard_tiles + idx)

    def reset(self, shim_magnets=True) -> None:
        """
