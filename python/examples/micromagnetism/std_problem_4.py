...
def std_prob_4(res=[64,16,1], NIST_field=1, cuda=False, show=True):
    ...
    if NIST_field == 1:
        ext_field = np.array([-24.6, 4.3, 0]) / 1000 / mu0
    elif NIST_field == 2:
        ext_field = np.array([-35.5, -6.3, 0]) / 1000 / mu0
    else:
        raise NotImplementedError()

    HextFct = lambda t: np.expand_dims(t > -1, 0).T * ext_field
