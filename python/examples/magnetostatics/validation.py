...
def valid_circpiece() -> None:
    ...
    validation("circpiece", tile_val, [0.406328622087633, 0.8631363102808783, 0.55])


def valid_circ_inv() -> None:
    tile_val = Tiles(
        n=1,
    ...
    validation(
        "circpiece_inverted", tile_val, [0.6271937452259475, 0.27251823835641853, 0.7]
    )


def valid_sphere() -> None:
    tile_val = Tiles(
        n=1,
        size=[0.25, 0, 0],
        offset=[0.3, 0.5, 0.1],
    ...
    validation("sphere", tile_val, [0.3, 0.4, 0.15], [0.25, 0.25, 0.75], unit="T")


def valid_spheroid() -> None:
    tile_val = Tiles(
        n=1,
        size=[0.5, 2, 0.5],
        offset=[0.1, 0.3, 0.45],
    ...
    validation("spheroid", tile_val, [0.3, 0.45, 0.2], [1, 0.75, 1], unit="T")


def valid_tetrahedron() -> None:
    tile_val = Tiles(
        n=1,
