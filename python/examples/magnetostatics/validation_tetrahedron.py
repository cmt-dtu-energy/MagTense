...
def main():
    ...
    tile_val.vertices = vert.transpose()
    tile_val.tile_type = 5
    tile_val.M_rem = 1.2 / (4*np.pi*1e-7)
    tile_val.u_ea = [0.324264068, 0.734846928, 0.891545179]
    ...
    (eval_points_x, H_norm_x_COMSOL) = load_COMSOL_eval(
        'Validation_tetrahedron_normH_x.txt', eval_offset, COMSOL_eval_path)
    (eval_points_y, H_norm_y_COMSOL) = load_COMSOL_eval(
        'Validation_tetrahedron_normH_y.txt', eval_offset, COMSOL_eval_path)
    (eval_points_z, H_norm_z_COMSOL) = load_COMSOL_eval(
        'Validation_tetrahedron_normH_z.txt', eval_offset, COMSOL_eval_path)
