...
def gen_s_state(
    ...
    problem_ini = MicromagProblem(
        res=res, grid_L=grid_size, m0=1 / np.sqrt(3), alpha=4.42e3, cuda=cuda
    )
...
def gen_seq(
    ...
    problem = MicromagProblem(
        res=res, grid_L=grid_size, m0=m0_state, alpha=4.42e3, gamma=2.211e5, cuda=cuda
    )
...
def load_demag_tensor(
    halbach: HalbachCylinder, pts_eval: EvaluationPoints, store: bool = False
) -> np.ndarray:
    """
    ...
    """
    fname = (
        f"N_{pts_eval.res[0]}_{pts_eval.res[1]}_{pts_eval.res[2]}_r_{pts_eval.r}_"
    ...
    if Path(fpath).is_file():
        ...
        if not store:
            print("[WARNING] No demagnetization tensor found for this configuration!")
            print("[RECOMMENDATION] Run `load_demag_tensor(store=True)` first.")
...
def calc_demag_field(
    ...
    (1) Demagnetization tensor N
        This might be pre-calculated from the existig structure
        and a full shim magnet matrix.
    (2) Iterating the magnetizations M of each single magnetic tile.
        This becomes especially computationally expensive,
        when a lof of shim magnets are present.
...
def get_shim_mat(
    mat: list | np.ndarray, layers: int, segs: int, symm: bool
) -> np.ndarray:
    """
    Args:
    ...
    if symm:
        ...
        for i in range(layers // 2):
            shim_mat[n_shim - (i + 1) * segs : n_shim - i * segs] = mat[
                i * segs : (i + 1) * segs
            ]
    else:
        shim_mat[:] = mat
...
def eval_shimming(
    halbach: HalbachCylinder, pts_eval: EvaluationPoints, log_p2p: bool = False
    ...
    b_norm = [
        np.linalg.norm(h_vec) * mu0
        for i, h_vec in enumerate(h)
        if (
            pts_eval.coords[i][0] ** 2
            + pts_eval.coords[i][1] ** 2
            + pts_eval.coords[i][2] ** 2
        )
        <= pts_eval.r**2
    ]
...
def plot_shim_matrix(
    ...
    if len(shim_mat) != n_fe:
        ...
        for i in range(n_layers // 2):
            mat[n_fe - ((i + 1) * n_segs) : n_fe - (i * n_segs)] = shim_mat[
                i * n_segs : (i + 1) * n_segs
            ]
        shim_mat = mat
...
def plot_halbach_line(
    field: np.ndarray,
    eval_pts: list[float] | np.ndarray,
    best_p2p: int | None = None,
) -> None:
    res = field.shape[1] // 3
    ...
    plt.legend(["x", "y", "z"])
    if best_p2p:
        plt.title(f"p2p: {best_p2p:.0f} ppm")
    plt.show()
