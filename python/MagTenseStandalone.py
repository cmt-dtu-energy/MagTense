...
# Input and output files required for MagTense defined in '../executable/io.txt'
...
class Tile():
    ...
    def set_mag_angle(self, theta):
        azimuth = theta[0]
        polar_angle = theta[1]
        self.set_easy_axis([np.sin(polar_angle) * np.cos(azimuth),
                            np.sin(polar_angle) * np.sin(azimuth),
                            np.cos(polar_angle)])
        self.set_oa1([np.sin(polar_angle) * np.sin(azimuth),
                      np.sin(polar_angle) * (-np.cos(azimuth)),
                      0])
        self.set_oa2([0.5 * np.sin(2 * polar_angle) * np.cos(azimuth),
...
class Grid():
    ...
    def get_tiles(self, grid_positions = [], n_tiles = None):
        ...
        if not grid_positions:
            if n_tiles is None:
                n_tiles = randrange(np.prod(self.places))
            else:
                n_tiles = n_tiles
            grid_positions = []
            while n_tiles > 0:
                new_pos = [randrange(self.places[0]-1),
                           randrange(self.places[1]-1),
                           randrange(self.places[2]-1)]
...
def setup(places, area, n_tiles=0, filled_positions=None, mag_angles=[], eval_points=[20, 20, 1], 
          eval_mode="uniform", B_rem=1.2, max_error=0.00001, max_it=500, iterate_solution=1, return_field=1):
    ...
    if not mag_angles:
        for i in range(0,len(tiles)):
            mag_angles = mag_angles.append(2 * np.pi * random())       
    for i, tile in enumerate(tiles):
        tile.set_remanence(B_rem / (4*np.pi*1e-7))
