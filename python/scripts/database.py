...
def db_magfield_symm(
    ...
    db.create_dataset('field', shape=out_shape, dtype="float32")
    db.create_dataset('scalar_potential', shape=(n_intv, 1, *res), dtype="float32")
    ...
    for idx in tqdm(range(n_intv)):
        ...
        filled_pos = [[i, j, k] 
                      ...
                      if filled_mat[intv[0] + idx][i][j][k] == 1 and (i < emp_x or i > emp_y
                        or j < emp_x or j > emp_y or k < (spots[2] // 2 - 1) or k > spots[2] // 2)]
        ...
        tiles = Tiles(
            ...
            color = [1, 0, 0],
            mag_angle=[[np.pi/2, 2*np.pi * mag_angle_mat[idx + intv[0], i]] for i in range(len(filled_pos))]
        )
