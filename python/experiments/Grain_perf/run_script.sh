...
echo -e "tag\tlabel\tN\tcpn\tifunif\tnlmin\tnlmax\tshort_circuit\tfmm_min_n\texit_code\tmax_rss_kb\tmax_gpu_mem_mib\tmesh_file" > "$summary"
...
len=${#BASES_LIST[@]}
for name in LABEL_LIST CPN_LIST IFUNIF_LIST NLMIN_LIST NLMAX_LIST ALLOW_FMM_SHORT_CIRCUIT_LIST FMM_MIN_N_LIST; do
  eval "this_len=\${#${name}[@]}"
...
for ((i=0; i<len; i++)); do
  ...
  tag="perf_${label}_N${N}_cpn${cpn}_ifunif${ifunif}_nl${nlmin}-${nlmax}_sc${allow_sc}_minN${fmm_min_n}_${timestamp}"
  ...
  cmd+=(--allow-fmm-short-circuit "$allow_sc" --fmm-min-n "$fmm_min_n")
