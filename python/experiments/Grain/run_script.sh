...
echo -e "tag\tN\tnGrains\tnRef\tdG\tcpn\tifunif\tnlmin\tnlmax\tshort_circuit\tfmm_min_n\texit_code\tmax_rss_kb\tmax_gpu_mem_mib\tmesh_file" > "$summary"
...
len=${#BASES[@]}
for name in CPN_LIST NGRAINS_LIST NREF_LIST DG_LIST IFUNIF_LIST NLMIN_LIST NLMAX_LIST ALLOW_FMM_SHORT_CIRCUIT_LIST FMM_MIN_N_LIST; do
  eval "this_len=\${#${name}[@]}"
...
for ((i=0; i<len; i++)); do
  ...
  echo -e "${tag}\t${N}\t${nGrains}\t${nRef}\t${dG}\t${cpn}\t${ifunif}\t${nlmin}\t${nlmax}\t${allow_sc}\t${fmm_min_n}\t${rc}\t${max_rss_kb}\t${max_gpu_mem_mib}\t${mesh_file}" >> "$summary"
