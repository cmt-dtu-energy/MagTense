include 'blas.f90'
   ...
   !Make a sparse matrix out of the dense matrices by specifying a threshold
    ...
    if ( grid%gridType .eq. gridTypeUniform ) then
        call OLD_ComputeExchangeTerm3D_Uniform( grid, A )
    ...
    !> which means it produces the differential operator d^2/dx^2 + d^2/dy^2 + d^2/dz^2 and returns this in the sparse matrix A
    !> Uses backwards and forwards finite difference on the edges
    !---------------------------------------------------------------------------   
    ...
    do k=1,nz
        do j=1,ny
            ...
            !The left boundary
            d2dx2%values(ind) = edgeVal
            d2dx2%cols(ind) = colInd
            d2dx2%rows_start(rowInd) = ind            
            ind = ind + 1
            
            d2dx2%values(ind) = neighbourEdgeVal
            d2dx2%cols(ind) = colInd + 1
            ind = ind + edgeIndInc

            d2dx2%values(ind) = nneighbourEdgeVal
            d2dx2%cols(ind) = colInd + 1 + edgeIndInc
    ...
    !!@todo Make hack for 1-element dimension less bad
    else ! bad hack
        allocate(d2dx2%values(1),d2dx2%cols(1),d2dx2%rows_start(nx*ny*nz),d2dx2%rows_end(nx*ny*nz))
        d2dx2%rows_start(1) = 1
        d2dx2%rows_end(1) = 1
        d2dx2%cols(1) = 1
        d2dx2%values(1) = 0.
    endif
    ...
    do k=1,nz
        ...
        do i=1,nx
            ...
            d2dy2%values(ind) = neighbourEdgeVal
            d2dy2%cols(ind) = colInd + nx
            
            !increment to next element
            ind = ind + edgeIndInc
            
            d2dy2%values(ind) = nneighbourEdgeVal
            d2dy2%cols(ind) = colInd + (1 + edgeIndInc) * nx
    ...
    !!@todo Make hack for 1-element dimension less bad
        ...
        allocate(d2dy2%values(1),d2dy2%cols(1),d2dy2%rows_start(nx*ny*nz),d2dy2%rows_end(nx*ny*nz))
        d2dy2%rows_start(1) = 1
        d2dy2%rows_end(1) = 1
        d2dy2%cols(1) = 1
        d2dy2%values(1) = 0.
    endif
    ...
    if ( nz .gt. 1 ) then
        ...
        do j=1,ny
            do i=1,nx
                ...
                !right-most value
                d2dz2%values(ind) = neighbourEdgeVal
                d2dz2%cols(ind) = nx * ny + colInd
                !increment position
                ind = ind + edgeIndInc
                
                d2dz2%values(ind) = nneighbourEdgeVal
                d2dz2%cols(ind) = ( 1 + edgeIndInc ) * nx * ny + colInd
        ...
        do j=1,ny
            do i=1,nx
                ...
                !left-most value
                d2dz2%values(ind) = nneighbourEdgeVal
                d2dz2%cols(ind) = colInd - ( 1 + edgeIndInc) * nx * ny
                d2dz2%rows_start(rowInd) = ind
    ...
    !> which means it produces the differential operator d^2/dx^2 + d^2/dy^2 + d^2/dz^2 and returns this in the sparse matrix A
    !---------------------------------------------------------------------------   
    subroutine RASMUS_ComputeExchangeTerm3D_Uniform( grid, A )
    type(MicroMagGrid),intent(in) :: grid             !> Struct containing the grid information    
    type(sparse_matrix_t),intent(inout) :: A           !> The returned matrix from the sparse matrix creator
        
    integer :: stat                                   !> Status value for the various sparse matrix operations        
    type(MagTenseSparse) :: d2dx2, d2dy2, d2dz2       !> Sparse matrices for the double derivatives with respect to x, y and z, respectively.
    type(sparse_matrix_t) :: tmp, tmp2, tmp3, tmp4                      !> Temporary sparse matrices used for internal calculations
    integer :: ind, ntot,colInd,rowInd                !> Internal counter for indexing, the total no. of elements in the current sparse matrix being manipulated    
    integer :: i,j,k,nx,ny,nz                         !> For-loop counters
    type(matrix_descr) :: descr                         !> Describes a sparse matrix operation
    real*4 :: const
    ...
    !----------------------------------d^2dx^2 begins -----------------------------!
    !Make the d^2/dx^2 matrix. The no. of non-zero elements is 3 * nx*ny*nz - 2 * ny * nz
    ntot = 3 * nx*ny*nz + 2*ny*nz !nz * ( ny * 4 + ny * (nx-2)*3 )
    allocate(d2dx2%values(ntot),d2dx2%cols(ntot),d2dx2%rows_start(nx*ny*nz),d2dx2%rows_end(nx*ny*nz))
    ...
    !----------------------------------d^2dy^2 begins ----------------------------!
    ntot = 3 * nx*ny*nz + 2*nx*nz !nz * ( nx * 2 + (ny-2) * nx * 3 + nx * 2 )
    ...
    do k=1,nz
        ...
        do i=1,nx
            ...
            d2dy2%values(ind) = 4.
            d2dy2%cols(ind) = colInd + 2*nx
            ind = ind + 1
            
            d2dy2%values(ind) = -1.
            d2dy2%cols(ind) = colInd + 3*nx
            d2dy2%rows_end(rowInd) = ind+1
            rowInd = rowInd + 1
    ...
    if ( nz .gt. 1 ) then
        ...
        ntot = 3 * nx*ny*nz !2 * 2 * nx * ny + 3 * (nz-2) * nx * ny
        !Make the d^2/dy^2 matrix. The no. of non-zero elements is 3 * nx*ny*nz - 2 * ny * nz just as for d^2dx^2
        allocate(d2dz2%values(ntot),d2dz2%cols(ntot),d2dz2%rows_start(nx*ny*nz),d2dz2%rows_end(nx*ny*nz))
    ...
    const = 1.
    stat = mkl_sparse_s_add (SPARSE_OPERATION_NON_TRANSPOSE, d2dx2%A, const, d2dy2%A, tmp)  
    !stat = mkl_sparse_s_add (SPARSE_OPERATION_TRANSPOSE, d2dx2%A, const, tmp3, tmp2)  
    !stat = mkl_sparse_s_add (SPARSE_OPERATION_TRANSPOSE, d2dy2%A, const, tmp2, tmp)    
    !stat = mkl_sparse_s_add (SPARSE_OPERATION_TRANSPOSE, tmp2, const, tmp3, tmp)    
    ...
    end subroutine RASMUS_ComputeExchangeTerm3D_Uniform
    ...
    !> which means it produces the differential operator d^2/dx^2 + d^2/dy^2 + d^2/dz^2 and returns this in the sparse matrix A
    !---------------------------------------------------------------------------   
    subroutine OLD_ComputeExchangeTerm3D_Uniform( grid, A )
    type(MicroMagGrid),intent(in) :: grid             !> Struct containing the grid information    
    type(sparse_matrix_t),intent(inout) :: A           !> The returned matrix from the sparse matrix creator
    ...
    end subroutine OLD_ComputeExchangeTerm3D_Uniform
